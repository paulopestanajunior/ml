# Checklist Geral do Plano 2

Use este checklist global para acompanhar sua evolução ao longo das oito semanas deste plano de estudos.  Marque cada item quando concluir todas as atividades da respectiva semana.

- [ ] **Semana 1 — Fundamentos de Data Science e Aprendizado de Máquina**
- [ ] **Semana 2 — Avaliação e Validação de Modelos**
- [ ] **Semana 3 — Aprendizado Não Supervisionado e Redução de Dimensionalidade**
- [ ] **Semana 4 — Redes Neurais e Deep Learning**
- [ ] **Semana 5 — Modelos de Linguagem e RAG**
- [ ] **Semana 6 — Sistemas Multiagentes: Fundamentos e Arquiteturas**
- [ ] **Semana 7 — Avaliação, Observabilidade e Métricas de Multiagentes**
- [ ] **Semana 8 — Implantação e Produção de Sistemas de LLM Multiagentes**

Ao marcar todos os itens, você terá percorrido todo o conteúdo programático do plano e estará preparado para construir, avaliar e colocar em produção sistemas de LLMs multiagentes.